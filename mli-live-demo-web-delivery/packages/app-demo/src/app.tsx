...
import { searchStore } from '@mli-csmo/base/dist/es/store/SearchStore'
    ...
    breadcrumbRender: (routers = []) => [
      {
        path: '/home',
        base: '/',
        breadcrumbName: <FormattedMessage id='micro.demo' />
      },
      ...routers
    ],
...
export function onRouteChange({ clientRoutes, location, basename, routes }) {
  ...
  if (route) {
    ...
    if (pathName !== MENU_LOCALE_BEGIN) {
      document.title = formatMessage({ id: pathName }) + ' mli-live-demo-web'
    }
  }

  // 切分segment，並將前四個的結果合在一起
  const pathSegments = location.pathname.split('/').filter(Boolean)
  const key = pathSegments.slice(0, 4).join('/')

  // 如果總結果有變化，重置searchStore的內容
