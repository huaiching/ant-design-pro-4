...
……以此類推
   ...
   default: ← 其他情況
      ...
      break;
        ...
        <pre><code>{`布林條件式 ? True的結果 : False的結果`}</code></pre>
        ...
        <pre><code>{`const data = (age>=18 ? '成年' : '未成年')`}</code></pre>
   ...
   內容
        ...
        <pre><code>{`const 變數 = 陣列變數.map((陣列元素變數) => {
   return 內容
})`}</code></pre>
...
export default VDOM`}
        ...
        <Paragraph type="secondary" style={{ marginTop: 32 }}>
          <Text strong>React 最佳實務提醒：</Text><br />
          • 渲染列表請務必使用 <Text code>map + key</Text>（不要用 for + push）<br />
          • 三元運算子非常適合簡單條件渲染<br />
          • 避免在 JSX 中寫複雜邏輯，保持畫面清晰
        </Paragraph>
