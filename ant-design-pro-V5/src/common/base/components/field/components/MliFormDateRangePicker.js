...
var MliFormDateRangePicker = function MliFormDateRangePicker(props) {
  ...
  defaultProps.formItemProps = _objectSpread({
    getValueProps: function getValueProps(value) {
      ...
      var defaultValue = value && value.map(function (item) {
        return item && (typeof item === 'string' ? dayjs(item, formatMessage({
          id: dayFormat
        })) : item);
      });
      return {
  ...
  return /*#__PURE__*/_jsx(ProFormDateRangePicker, _objectSpread({
    colProps: {
  ...
  }, omit(defaultProps, ['moduleName', 'columnName', 'colSpan', 'startColumnName', 'endColumnName'])));
};
