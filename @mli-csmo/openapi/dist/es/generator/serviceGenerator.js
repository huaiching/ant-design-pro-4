...
export var getGenInfo = function getGenInfo(isDirExist, appName, absSrcPath) {
  // dir 不存在，則沒有佔用，且為第一次
  if (!isDirExist) {
    return [false, true];
  }
  var indexList = glob.sync("@(".concat(BASE_DIRS.join('|'), ")/").concat(appName, "/index.@(js|ts)"), {
    cwd: absSrcPath
  });
  // dir 存在，且 index 存在
  if (indexList && indexList.length) {
    var indexFile = join(absSrcPath, indexList[0]);
    try {
      var line = (readFileSync(indexFile, 'utf-8') || '').split(/\r?\n/).slice(0, 3).join('');
  ...
  return [true, !(existsSync(join(absSrcPath, BASE_DIRS[0], appName, 'openAPI')) || existsSync(join(absSrcPath, BASE_DIRS[1], appName, 'openAPI')))];
};
var DEFAULT_SCHEMA = {
  type: 'object',
  properties: {
...
var ServiceGenerator = /*#__PURE__*/function () {
  ...
  _createClass(ServiceGenerator, [{
    ...
    value: function genFile() {
      ...
      try {
        var finalPath = join(basePath, this.config.projectName || '');
        this.finalPath = finalPath;
        glob.sync("".concat(finalPath, "/**/*")).filter(function (ele) {
          return !ele.includes('_deperated');
        }).forEach(function (ele) {
          rimraf.sync(ele);
        });
      } catch (error) {
        Log("\uD83D\uDEA5 serves \u751F\u6210\u5931\u6557: ".concat(error));
      }
    ...
    value: function getFuncationName(data) {
      ...
      return this.config.hook && this.config.hook.customFunctionName ? this.config.hook.customFunctionName(data) : data.operationId ? this.resolveFunctionName(stripDot(data.operationId), data.method) : data.method + this.genDefaultFunctionName(data.path, pathBasePrefix);
    }
    ...
    value: function getTemplate(type) {
      return readFileSync(join(this.config.templatesFolder || '', "".concat(type, ".njk")), 'utf8');
    }
